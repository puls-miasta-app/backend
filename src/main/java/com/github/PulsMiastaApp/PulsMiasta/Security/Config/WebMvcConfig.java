 ...
 *
 * <p>{@link SudoModeFilter} runs as an interceptor (not a servlet filter) so that
 * Spring MVC has already resolved the handler method when the check executes.
...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(sudoModeFilter);
    }
}
